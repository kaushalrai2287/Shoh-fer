...
interface Role {
  role_id: string;
  role_name: string;
  permissions: string[]; // Added permission names here
}
...
const RollsPermission = () => {
  ...
  useEffect(() => {
    const fetchRoles = async () => {
      try {
        const response = await fetch(`/api/roles/getrolls?role_name=${roleNameFilter}`);
        if (!response.ok) {
          throw new Error("Failed to fetch roles");
        }
        ...
        const updatedRoles = await Promise.all(roles.map(async (role: Role) => {
          const supabase = createClient();

          // Fetch permissions from the role_permissions table
          ...
          if (rolePermissionsError) {
            console.error(rolePermissionsError);
  ...
  const columns = {
    Role_Name: "Role Name",
    Permissions: "Permissions", // Add permissions to columns
  };
  ...
  const data = roles.map((role) => ({
    Role_Name: role.role_name,
    Permissions: role.permissions.join(", "), // Join permission names for display
    onDelete: () => handleDelete(role.role_id),
  }));
              ...
              <form onSubmit={(e) => e.preventDefault()}>
                ...
                <div className="inner_form_group inner_form_group_submit">
                  <input
                    ...
                    className="submite_btn"
                    value="Search"
                    onClick={() => setRoleNameFilter(roleNameFilter)}
                  />
                  <CSVLink
                    data={roles}
                    headers={headers}
