  ...
  try {
      const reqStream = new Readable();
        ...
        const form = new multiparty.Form({
          uploadDir: path.join(process.cwd(), "public/uploads/DriverLicence"),
          autoFiles: true,
        });
    ...
    const LicenceUrl = `/uploads/DriverLicence/${newFileName}`;
    // console.log('Profile photo URL:', profilePhotoUrl);

    // ✅ Update the driver's record in Supabase
    const { error } = await supabase
      .from('drivers')
      .update({ driving_license_image: LicenceUrl })
